...
class LaczeDoGNUPlota {
 ...
 public:
   ...
   */
  void SetDrawingMode(TypTrybuRysowania  Tryb) { _TrybRys = Tryb; }
   ...
   */
  void SetRangeX(float Xo, float Xn) { _Xmin = Xo;  _Xmax = Xn; }
  /*!
   ...
   */
  void SetRangeY(float Yo, float Yn) { _Ymin = Yo;  _Ymax = Yn; }
  /*!
   ...
   */
  void SetRangeZ(float Zo, float Zn) { _Zmin = Zo;  _Zmax = Zn; }
  ...
  void WyswietlajKomunikatyBledow( bool  WyswietlajKomunikaty = true );
    ...
    */
    InfoPlikuDoRysowania& AddFilename(
   ...
   * \brief Usuwa z listy nazw plików z danymi do rysowania usuwa wybraną nazwę.
   */
  bool DeleteFileName( const char          *sNazwaPliku,
		       TypDostepuDoZasobu   Dostep = DZ_Lokalny
		      );
